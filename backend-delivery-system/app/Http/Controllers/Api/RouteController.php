...
class RouteController extends Controller
{
    ...
    public function show(DeliveryOrder $deliveryOrder): JsonResponse
    {
        ...
        if (! $route) {
            return response()->json([
                'success' => false,
                'message' => 'No route found for this order.',
            ], 404);
        }

        return response()->json([
            'success' => true,
            'data'    => ['route' => $route, 'stops' => $deliveryOrder->stops],
        ]);
    }
    ...
    public function reOptimize(DeliveryOrder $deliveryOrder): JsonResponse
    {
        ...
        if (! in_array($deliveryOrder->status, ['assigned', 'in_transit'])) {
            return response()->json([
                'success' => false,
                'message' => 'Route can only be re-optimized for assigned or in-transit orders.',
            ], 422);
        }
    ...
    public function eta(DeliveryOrder $deliveryOrder): JsonResponse
    {
        ...
        $eta = $this->routeService->calculateETA(
            $driver->current_lat,
            $driver->current_lng,
            $deliveryOrder->delivery_lat,
            $deliveryOrder->delivery_lng
        );
