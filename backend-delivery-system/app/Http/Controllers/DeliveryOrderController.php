...
class DeliveryOrderController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json(['success' => true, 'data' => $query->latest()->paginate($request->per_page ?? 15)]);
    }
    ...
    public function store(StoreOrderRequest $request): JsonResponse
    {
        ...
        return response()->json(['success' => true, 'message' => 'Order created successfully.', 'data' => $order->load(['stops', 'creator'])], 201);
    }
    ...
    public function update(UpdateOrderRequest $request, DeliveryOrder $deliveryOrder): JsonResponse
    {
        ...
        if (in_array($deliveryOrder->status, ['delivered', 'cancelled'])) {
            return response()->json(['success' => false, 'message' => 'Cannot update a completed or cancelled order.'], 422);
        }

        $deliveryOrder->update($request->validated());
        return response()->json(['success' => true, 'message' => 'Order updated successfully.', 'data' => $deliveryOrder->fresh(['stops', 'driver'])]);
    }
    ...
    public function assignDriver(Request $request, DeliveryOrder $deliveryOrder): JsonResponse
    {
        ...
        $deliveryOrder->update(['driver_id' => $request->driver_id, 'status' => 'assigned', 'assigned_at' => now()]);
        ...
        return response()->json([
            ...
            'data'    => ['order' => $deliveryOrder->fresh(['driver.user', 'route']), 'route' => $route],
        ]);
    }
    ...
    public function updateStatus(Request $request, DeliveryOrder $deliveryOrder): JsonResponse
    {
        ...
        if (!in_array($request->status, $allowed)) {
            return response()->json(['success' => false, 'message' => "Cannot transition from '{$deliveryOrder->status}' to '{$request->status}'."], 422);
        }

        $oldStatus = $deliveryOrder->status;
        $updates   = ['status' => $request->status];

        if ($request->status === 'in_transit') $updates['pickup_at'] = now();
        if ($request->status === 'delivered')  { $updates['delivered_at'] = now(); $updates['actual_duration'] = $deliveryOrder->calculateActualDuration(); }
        if ($request->delay_reason)            $updates['delay_reason'] = $request->delay_reason;
        ...
        return response()->json(['success' => true, 'message' => 'Order status updated.', 'data' => $deliveryOrder->fresh()]);
    }
    ...
    public function cancel(Request $request, DeliveryOrder $deliveryOrder): JsonResponse
    {
        ...
        if (in_array($deliveryOrder->status, ['delivered', 'cancelled'])) {
            return response()->json(['success' => false, 'message' => 'Order is already completed or cancelled.'], 422);
        }
    ...
    private function authorizeOrderAccess(DeliveryOrder $order): void
    {
        $user = auth()->user();
        if ($user->role === 'driver') {
            abort_if($order->driver_id !== $user->driver?->id, 403, 'Unauthorized access to this order.');
        }
    }
